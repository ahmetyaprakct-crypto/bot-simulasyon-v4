...
def is_range_market_by_price(candles, liq_candle_time, window=20):
    idx = None
...
def run_confirmation_chain(
    ...
    for vf in valid_fractals:
        ...
        is_in_range = None
        if liq_fractal_idx is not None:
            window = 20
            if liq_fractal_idx >= window:
                prev_candles = candles[liq_fractal_idx-window:liq_fractal_idx]
                range_high = max(c['high'] for c in prev_candles)
                range_low = min(c['low'] for c in prev_candles)
                range_flag = True
                liq_candle = next((c for c in candles if c['open_time'] == vf['liquidity_time']), None)
        ...
        if entry_found and not trade_outcome:
            # Olası edge-case: TP/SL olmadan, candle'lar bitmişse
            # Son kapanışa göre outcome belirle (isteğe göre SL/TP veya None bırakılabilir)
            # Burada SL veya TP yerine None da bırakabilirsin.
            trade_outcome = 'SL'  # veya 'TP' ya da None (sana bağlı, çoğu durumda SL seçilir)

...
structure_timeframe = '15m'  # 1h market structure için kullanılacak
...
def call_detect_fractals(symbol, tf, n=2, start_str=None, end_str=None):
    ...
    fractal_detector.detect_fractals(symbol, candles, db, tf, n=n)
    db.close()
...
def call_check_liquidity(symbol, tf, start_str=None, end_str=None):
    ...
    with db.conn.cursor() as cur:
        ...
        fractals = [{
            'fractal_time': r[0],
            'fractal_type': r[1],
            'fractal_price': float(r[2]),
            'candle_high': float(r[3]) if r[3] is not None else None,
            'candle_low': float(r[4]) if r[4] is not None else None
        } for r in rows]
...
def get_active_trend(trend_log, time_5m):
    if not time_5m or time_5m == "":
        return ""
    try:
        # Trend zamanlarını datetime'a çevir
        times = [
            t[0] if isinstance(t[0], datetime) else
            datetime.fromisoformat(str(t[0]).replace('Z','')) if "-" in str(t[0]) else
            datetime.strptime(str(t[0]), "%d.%m.%Y %H:%M")
            for t in trend_log
        ]
...
def full_gridsearch(trade_logs, start_str, end_str):
    ...
    for symbol in symbols:
        ...
        call_detect_fractals(symbol, structure_timeframe, n=10, start_str=start_str, end_str=end_str)
    ...
    with db.conn.cursor() as cur:
        ...
        fracts_1h = [{'fractal_time': row[0], 'fractal_type': row[1], 'price': float(row[2])} for row in cur.fetchall()]
    db.close()
    ...
    if len(trade_logs) > 0 and isinstance(trade_logs[0], dict):
        ...
        df_trades.to_csv(f"logs/trade_log_with_trend_{symbols[0]}_{start_str[:10]}_{end_str[:10]}.csv", index=False)
        print(f"✅ trade_log_with_trend_{symbols[0]}_{start_str[:10]}_{end_str[:10]}.csv dosyasına 1h trend bilgisi eklendi!")
...
if __name__ == "__main__":
    start_str = "2025-07-01 00:00:00"
    end_str   = "2025-07-17 00:00:00"
    ...
    for i, (chunk_start, chunk_end) in enumerate(chunk_list, 1):
        ...
        # trade_logs ya da detailed_logs liste halinde olacak
        if detailed_logs:
            import pandas as pd
